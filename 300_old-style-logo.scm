...
;(gimp-context-set-gradient (list-ref (cadr (gimp-gradients-get-list "")) 1))
			(if (= use-second-col? FALSE)	(begin (if (not (defined? 'gimp-drawable-filter-new)) 
...
(gimp-context-set-gradient (vector-ref (car (gimp-gradients-get-list ""))6))))		
		(begin (if (not (defined? 'gimp-drawable-filter-new)) 
		(gimp-context-set-gradient (list-ref (cadr (gimp-gradients-get-list "")) 1)) 
...
(script-fu-register "script-fu-old-style-logo-300"
		    "Old Style Logo 300"
