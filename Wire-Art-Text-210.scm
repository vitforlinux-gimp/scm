...
(define (apply-wire-art-chrome-logo-effect img
                                  logo-layer
                                  offsets
                                  bg-color)
  (let* (
...
(define (script-fu-wire-art-text-old text
				just
				lspace
				space 
				font-in 
...
(script-fu-wire-art-textcore image img-width img-height text-layer text-width text-height
				grow
				shape 
				grid-type
				glid-size
...
(script-fu-register "script-fu-wire-art-text-old"
  "Wire Art Text Old"
...
;;;  SF-COLOR	"Background color"	"lightgrey"		;none 2.4
  ...
  SF-TOGGLE	"Bacground to Image only"	FALSE  
	...
	"script-fu-wire-art-text-old"
	"<Image>/Script-Fu/Graechan/Logos/"
)

(define (script-fu-wire-art-textimg img t-layer
				grow
				shape 
				grid-type
				glid-size
...
(script-fu-wire-art-textcore img img-width img-height t-layer text-width text-height
				grow
				shape 
				grid-type
				glid-size
...
;画像加工本体
(define (script-fu-wire-art-textimgh img t-layer
				grow
				shape 
				grid-type
				glid-size
  ...
  (let* (
	...
	(script-fu-wire-art-textimg img2 drawable2
				grow
				shape 
				grid-type
				glid-size
...
(script-fu-register "script-fu-wire-art-textimgh"
  "Wire Art Image"
  "Creates text made fom wire"
  "Graechan"
  "Graechan"
...
;;;  SF-COLOR	"Background color"	"lightgrey"		;none 2.4
  ...
  (script-fu-menu-register
	"script-fu-wire-art-textimgh"
	"<Image>/Script-Fu/Graechan/AlphaToLogo/"
)

(define (script-fu-wire-art-textcore image img-width img-height text-layer text-width text-height
				grow
				shape 
				grid-type
				glid-size
...
;;;;create channel
	...
	(if (= 0 grid-type) (plug-in-mosaic 1 image bkg-layer glid-size 1 grid-wire-size 1 0 135 0 1 0 0 1 1 ))
	(if (= 1 grid-type) (plug-in-mosaic 1 image bkg-layer glid-size 1 grid-wire-size 1 0 135 0 1 0 1 1 1 ))
	(if (= 3 grid-type) (plug-in-mosaic 1 image bkg-layer glid-size 1 grid-wire-size 0.5 0 135 0 1 0 1 1 1 ))
	(gimp-image-select-item image 2  text-layer)
;;;;create channel
	(gimp-selection-save image)
	...
	(apply-wire-art-chrome-logo-effect image text-layer 10 bkg-color)
