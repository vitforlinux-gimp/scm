...
(script-fu-register
            "script-fu-freaky-text-300"                        ;function name
            "Freaky Text 300"                                  ;menu label
            "Creates a Freaky text, sized to fit\
            ...
            SF-FONT        "Font"          "QTAtchen Medium"    ;a font variable
            SF-ADJUSTMENT  "Font size"     '(150 1 1000 1 10 0 0)
	     SF-COLOR       "Color"         '(30 30 30)     ;color variable
...
SF-ADJUSTMENT _"Distortion Diffusion"          '(4 0 20 1 10 0 0);a spin-button
SF-ADJUSTMENT _"Distortion Granularity"          '(3 0 20 1 10 0 0);a spin-button
SF-ADJUSTMENT _"Distortion Smoothing"          '(2 0 20 1 10 0 0);a spin-button

            SF-ADJUSTMENT  "Shadow Lenght"     '(10 1 1000 1 10 0 0)
           SF-COLOR       "Extrusion Color"         '(255 0 0)     ;color variable
           SF-COLOR       "Background Color"         '(255 255 255)     ;color variable
            SF-ADJUSTMENT  "Buffer amount" '(35 0 100 1 10 1 0)
                                                        ;a slider
							SF-TOGGLE   "Merge Layers"     TRUE
  )
  (script-fu-menu-register "script-fu-freaky-text-300" "<Image>/File/Create/Text")
...
(define (script-fu-freaky-shine image drawable
                              shadow-size
							  shadow-opacity
							  keep-selection-in
							  conserve)
...
(if (not (defined? 'gimp-drawable-filter-new))
 ...
 (if (= conserve FALSE)
  ...
  (define (script-fu-freaky-text-300 inText inFont inFontSize inTextColor
					justification
					letter-spacing
					line-spacing
					grow-text
...
(script-fu-distress-selection theImage (vector theText) 0.9 DistDiff DistGran DistSmoo TRUE TRUE)
(gimp-selection-feather theImage 1)
(gimp-drawable-edit-fill theText FILL-FOREGROUND)
		          (script-fu-freaky-shine theImage theText
                (round (/ inFontSize 5))
				0
