...
; Accès 
; File > Create > Logos > Brutalist
; Filters > Alpha to logo > Brutalist ...
;
;
...
(define
	(
		apply-brutalist300-logo-effect
		img
		basetext
		t-colored
		text-color
...
"pct-random" 50 "repeat" 1 "seed" 0 ) (gimp-drawable-desaturate fond 4))		    
    ...
    (else
		...
		(if (= fplasma TRUE)
				    (cond((not(defined? 'plug-in-plasma))
		 		     (gimp-drawable-merge-new-filter basetext "gegl:plasma" 0 LAYER-MODE-REPLACE 1.0
"turbulence" 1.0 "x" 0 "y" 0 "width" width "height" height "seed" 0 ) (gimp-drawable-desaturate basetext 4))		    
	(else
		(plug-in-plasma 1 img basetext 0 1.0)(gimp-drawable-desaturate basetext 4))))
		(if (= fnoise TRUE)
				    (cond((not(defined? 'plug-in-noisify))
		 		     (gimp-drawable-merge-new-filter basetext "gegl:noise-hurl" 0 LAYER-MODE-REPLACE 1.0
"pct-random" 50 "repeat" 1 "seed" 0 ) (gimp-drawable-desaturate basetext 4))		    
	(else
		(plug-in-noisify 1 img basetext 1 0.2 0.2 0.2 0))))
(if (= t-colored TRUE) (begin 	(gimp-context-set-paint-mode LAYER-MODE-MULTIPLY)	(gimp-context-set-foreground text-color)
		(gimp-drawable-edit-fill basetext FILL-FOREGROUND)(gimp-context-set-paint-mode LAYER-MODE-NORMAL) ) )
...
(define
	(
		script-fu-brutalist300-logo-alpha
		img
		text-layer
		t-colored
		text-color
	...
	(begin
		...
		(apply-brutalist300-logo-effect img text-layer t-colored text-color boolrippleh boolripplev fnoise fplasma bnoise bplasma colored bcolor)
...
(script-fu-register
	"script-fu-brutalist300-logo-alpha"
	"Brutalist 300 Alpha..."
	"Create a Brutalist Concrete alpha"
	...
	SF-TOGGLE 	"Shape colored" FALSE
	SF-COLOR 	"Shape Color" '(200 200 40)
	SF-TOGGLE	"Ripple Horiz." FALSE
	SF-TOGGLE	"Ripple Vert."  FALSE
	SF-TOGGLE      "Front Noise" TRUE
	SF-TOGGLE      "Front Plasma." TRUE
...
(script-fu-register
	"script-fu-brutalist300-logo"
	"Brutalist 300 Logo"
	"Create a Brutalist Concrete logo"
	"Denis Bodor <lefinnois@lefinnois.net>"
	"Denis Bodor"
	"03/31/2005"
	""
	SF-TEXT "Enter your text" "Brutalist"
	SF-FONT "Font Name" "QTHelvet-Black Black"
  ...
  SF-ADJUSTMENT _"Outline"          '(0 0 40 1 10 0 0)
	SF-TOGGLE 	"Text colored" FALSE
	SF-COLOR "Text Color" '(200 200 40)
	SF-TOGGLE "Ripple Horiz." FALSE
	SF-TOGGLE "Ripple Vert." FALSE
	SF-TOGGLE "Text Noise" TRUE
	SF-TOGGLE "Text Plasma." TRUE
	SF-TOGGLE "Back Noise" TRUE
	SF-TOGGLE "Back Plasma." TRUE
	SF-TOGGLE "Background colored" FALSE
	SF-COLOR   "Background  color"         '(255 104 0)
