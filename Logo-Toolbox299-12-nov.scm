...
(gimp-context-set-gradient-reverse rev-blend)
	...
	(gimp-image-select-rectangle img 0 0 0 1 1)
	    (plug-in-plasma 1 img fill-layer (rand 999999999) (+ 1 (random 3))) ; Rnd Plasma Fill
    ...
    (if (= fill-style 7) ; Diffraction Fill
        (begin  
             ...
             ;(plug-in-diffraction 1 img fill-layer .815 1.221 1.123 (+ .821 (rand 2)) (+ .821 (rand 2)) (+ .974 (rand 2)) .610 .677 .636 .066 (+ 27.126 (rand 20)) (+ -0.437 (rand 2)))
              (plug-in-diffraction 1 img fill-layer .815 1.221 1.123 (+ .821 (rand 2)) (+ .821 (rand 2)) (+ .974 (rand 2)) .610 .677 .636 .066 (+ 27.126 (rand 20)) (+ -0.437 (rand 1)))              
...
;
(define (script-fu-logo-toolbox299-alpha image
                                       layer
									   create-new
                                       fill-style
                                       logo-color
...
;
(script-fu-register "script-fu-logo-toolbox299-alpha"
                    _"_Logo Toolbox299 alpha..."
                    ...
                    SF-PATTERN    _"Background pattern"   "Paper"
...
(script-fu-menu-register "script-fu-logo-toolbox299-alpha"
                         "<Image>/Filters/Alpha to Logo")

;
; Define logo text procedure
;
(define (script-fu-logo-toolbox299 text
                                 font
                                 size
								 direction
								 justification
...
; Register the script
;    
(script-fu-register "script-fu-logo-toolbox299"
                    _"_Logo Toolbox299.."
...
(script-fu-menu-register "script-fu-logo-toolbox299"
