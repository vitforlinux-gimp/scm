...
;
 ...
 (let* (
			...
			(bkg-layer (cond ((= (string->number (substring (car(gimp-version)) 0 3)) 2.10)
			...
			(chrome (cond ((= (string->number (substring (car(gimp-version)) 0 3)) 2.10)
			(car (gimp-layer-new image width height RGBA-IMAGE "Chrome" 100 LAYER-MODE-NORMAL-LEGACY)))
			(else (car (gimp-layer-new image "Chrome" width height RGBA-IMAGE 100 LAYER-MODE-NORMAL-LEGACY)))))
