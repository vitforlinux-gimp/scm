...
(define (script-fu-double-bump-map-300 image layer 
              color
			  pattern
			  no-pattern
              grow-distance
	...
	(let* 
		...
		(apply-gauss2 image layer
											blur-radius blur-radius
											;1 ;1 RLE or 0 IIR
											)
											
		;creates new colored layer to bump map
		(set! color-layer (car (gimp-layer-new-ng image image-width image-height
		...
		(set! pattern-layer (car (gimp-layer-new-ng image image-width image-height
	...
	(cond((not(defined? 'plug-in-bump-map))
	    (let* ((filter (car (gimp-drawable-filter-new color-layer "gegl:bump-map" ""))))
      (gimp-drawable-filter-configure filter LAYER-MODE-REPLACE 1.0
                                      ...
                                      "tiled" FALSE)
      (gimp-drawable-filter-set-aux-input filter "aux" layer)
      (gimp-drawable-merge-filter color-layer filter)
    ))
    (else
	...
	(cond((not(defined? 'plug-in-bump-map))
	    (let* ((filter (car (gimp-drawable-filter-new pattern-layer "gegl:bump-map" ""))))
      (gimp-drawable-filter-configure filter LAYER-MODE-REPLACE 1.0
                                      "azimuth" bump-azimuth "elevation" bump-elevation "depth" bump-depth
                                      "offset-x" 0 "offset-y" 0 "waterlevel" 0.0 "ambient" 0
                                      "compensate" TRUE "invert" FALSE "type" "linear"
                                      "tiled" FALSE)
      (gimp-drawable-filter-set-aux-input filter "aux" layer)
      (gimp-drawable-merge-filter pattern-layer filter)
    ))
    (else
...
(script-fu-register
  "script-fu-double-bump-map-300"         ;function name
...
(script-fu-menu-register "script-fu-double-bump-map-300" "<Image>/Script-Fu/Alpha-to-Logo/")


  (script-fu-register
            "script-fu-double-bump-text-300"                        ;function name
            "Double Bumped Text 300"                                  ;menu label
            ...
            SF-FONT        "Font"          "QTLautrecType Medium"    ;a font variable
	...
	(gimp-image-select-item theImage 2 theText)
     ...
     ;(gimp-image-resize-to-layers theImage)
      ...
      (gimp-display-new theImage)
                  (script-fu-double-bump-map-300 theImage theText 
              color
			  pattern
			  no-pattern
              grow-distance
