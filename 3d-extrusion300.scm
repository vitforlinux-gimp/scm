...
; V 300 - Fix for Gimp 3.0 rc2 (not complete) & 2.10
; Define 3D Extrude Aplha
;
(define (script-fu-3d-extrude300 img
                              drawable
                              ext-type
                              ext-depth
                              ext-direction
...
;
	(cond((not(defined? 'plug-in-bump-map))
	    (let* ((filter (car (gimp-drawable-filter-new in-layer "gegl:bump-map" ""))))
...
) ; return (from script-fu-3d-extrude300) 
