...
(define (script-fu-skywriting inText inSize inFont  justification letter-spacing line-spacing grow-text outline color foption scolor grad pat)
	...
	(define oldBrush (car (gimp-context-get-brush)))
	(define old-grad (car (gimp-context-get-gradient)))
        (define old-pat (car (gimp-context-get-pattern)))
...
(let* (
   ...
   (gimp-text-layer-set-line-spacing theText line-spacing)      ; Set Line Spacing    
	...
	(gimp-layer-resize theText (+(car (gimp-drawable-width theText)) buffer buffer ) (+ (car (gimp-drawable-height theText)) buffer buffer ) 0 0)	;add
	(gimp-image-resize img (car (gimp-drawable-width theText)) (car (gimp-drawable-height theText)) 0 0)		;add
   ;;;;;; SHRINK/GROW text
(cond ((> grow-text 0)
...
;	(gimp-layer-delete theText)		;none 2.2
  ...
  (cond((= foption 1)
    (gimp-gradients-set-gradient grad)
