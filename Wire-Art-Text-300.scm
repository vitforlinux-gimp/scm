...
; Rel 005 / 300 update code for gimp 3.0 rc2 +git and 2.10 compat off
...
(define (apply-wire-art-300-chrome-logo-effect img ; qui
  ...
  (let* (
        ...
        (layer1 (car (gimp-layer-new-ng img width height RGBA-IMAGE "Layer 1" 100 LAYER-MODE-DIFFERENCE-LEGACY)))
        (layer2 (car (gimp-layer-new-ng img width height RGBA-IMAGE "Layer 2" 100 LAYER-MODE-DIFFERENCE-LEGACY)))
        (layer3 (car (gimp-layer-new-ng img width height RGBA-IMAGE "Layer 3" 100 LAYER-MODE-NORMAL-LEGACY)))
        (shadow (car (gimp-layer-new-ng img width height RGBA-IMAGE "Drop Shadow" 100 LAYER-MODE-NORMAL-LEGACY)))
        (background (car (gimp-layer-new-ng img width height RGB-IMAGE "Background" 100 LAYER-MODE-NORMAL-LEGACY)))
...
(define (script-fu-layers-view-current-only image drawable)
    ...
    (if (not (= 0 (car (gimp-item-is-layer drawable))))
...
(define (script-fu-wire-art-300-text text
				just
				lspace
				space 
				font-in 
...
(gimp-context-set-paint-mode 0)
(script-fu-wire-art-300-textcore image img-width img-height text-layer text-width text-height
				grow
				shape 
				grid-type
				glid-size
...
(script-fu-register "script-fu-wire-art-300-text"
  "Wire Art Text 300"
  ...
  SF-OPTION	"Grid Type"			'("Squared" "Hexagonal" "None" "Randomized")
...
;;;  SF-COLOR	"Background color"	"lightgrey"		;none 2.4
  ...
  SF-TOGGLE	"Bacground to Image only"	FALSE  
	...
	"script-fu-wire-art-300-text"
	"<Image>/Script-Fu/Logos/"
)

(define (script-fu-wire-art-300-textimg img t-layer
				grow
				shape 
				grid-type
				glid-size
...
(gimp-context-set-paint-mode 0)
(script-fu-wire-art-300-textcore img img-width img-height t-layer text-width text-height
				grow
				shape 
				grid-type
				glid-size
...
;画像加工本体
(define (script-fu-wire-art-300-textimgh img t-layer
				grow
				shape 
				grid-type
				glid-size
  ...
  (let* (
	...
	;			))
	(cond ((defined? 'gimp-image-get-selected-layers) (set! drawable2 (vector-ref (car (gimp-image-get-selected-drawables img2)) 0))	)
(else (set! drawable2 (car (gimp-image-get-active-drawable img2)))
	)	)
	...
	(script-fu-wire-art-300-textimg img2 drawable2
				grow
				shape 
				grid-type
				glid-size
...
(script-fu-register "script-fu-wire-art-300-textimgh"
  "Wire Art Image 300"
  "Creates text made fom wire"
  "Graechan"
  "Graechan"
...
;;;  SF-COLOR	"Background color"	"lightgrey"		;none 2.4
  ...
  (script-fu-menu-register
	"script-fu-wire-art-300-textimgh"
	"<Image>/Script-Fu/Alpha-to-Logo/"
)

(define (script-fu-wire-art-300-textcore image img-width img-height text-layer text-width text-height
				grow
				shape 
				grid-type
				glid-size
...
(cond((not(defined? 'plug-in-mosaic))
    (gimp-drawable-merge-new-filter bkg-layer "gegl:mosaic" 0 LAYER-MODE-REPLACE 1.0
    "tile-type" "squares" "tile-size" glid-size "tile-height" 1 "tile-neatness" 1 "color-variation" 0 "color-averaging" TRUE "tile-surface" 0 "tile-allow-split" FALSE "tile-spacing" grid-wire-size "joints-color" '(0 0 0) "light-color" '(1 1 1) "light-dir" 135 "antialiasing" TRUE "seed" 0) 
    )
(else
	(plug-in-mosaic 1 image bkg-layer glid-size 1 grid-wire-size 1 0 135 0 1 0 0 0 1 ))))
	;(if (= 0 grid-type)(plug-in-grid 1 image (vector bkg-layer) 2 glid-size 0 "Black"  2 glid-size 0  "Black"  2 2 0 "Black"))
	(if (= 1 grid-type)
(cond((not(defined? 'plug-in-mosaic))
    (gimp-drawable-merge-new-filter bkg-layer "gegl:mosaic" 0 LAYER-MODE-REPLACE 1.0
    "tile-type" "hexagons" "tile-size" glid-size "tile-height" 1 "tile-neatness" 1 "color-variation" 0 "color-averaging" TRUE "tile-surface" 0 "tile-allow-split" FALSE "tile-spacing" grid-wire-size "joints-color" '(0 0 0) "light-color" '(1 1 1) "light-dir" 135 "antialiasing" TRUE "seed" 0) 
    )
(else
	(plug-in-mosaic 1 image bkg-layer glid-size 1 grid-wire-size 1 0 135 0 1 0 1 0 1 ))))
	(if (= 3 grid-type)
(cond((not(defined? 'plug-in-mosaic))
    (gimp-drawable-merge-new-filter bkg-layer "gegl:mosaic" 0 LAYER-MODE-REPLACE 1.0
    "tile-type" "hexagons" "tile-size" glid-size "tile-height" 1 "tile-neatness" 0.5 "color-variation" 0 "color-averaging" TRUE "tile-surface" 0 "tile-allow-split" TRUE "tile-spacing" grid-wire-size "joints-color" '(0 0 0) "light-color" '(1 1 1) "light-dir" 135 "antialiasing" TRUE "seed" 0) 
    )
(else
	(plug-in-mosaic 1 image bkg-layer glid-size 1 grid-wire-size 0.5 0 135 0 1 0 1 0 1 ))))
	(gimp-image-select-item image 2  text-layer)
;;;;create channel
	(gimp-selection-save image)
...
(else (gimp-image-set-active-layer image text-layer))
    ...
    (if (> metal 0)
    (begin 
	(set! tint-layer (car (gimp-layer-new-ng image width height RGBA-IMAGE "Tint" 100 LAYER-MODE-MULTIPLY-LEGACY)))
